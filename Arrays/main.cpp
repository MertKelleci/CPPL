...
int main()
{
	...
	// Arrays created on the heap can be allocated dynamically, meaning you can pass a variable as the size of the array, which allows for more flexible and dynamic memory management. On the other hand, arrays created on the stack must have a fixed size that is known at compile time, which can limit their flexibility and make it difficult to work with data that has a variable size. Therefore the following wouldn't work:
	// int array[size];
	int array[5];
